...
def remove_small_sentences(df, column = 'text', min_words = 3):
    """
    Removes sentences where the specified column contains sentences with fewer than min_words
    """
    return df[df[column].apply(lambda x: len(x.split()) >= min_words)].reset_index(drop=True)